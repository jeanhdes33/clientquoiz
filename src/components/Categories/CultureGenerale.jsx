...
function CultureGenerale() {
  ...
  const fetchCultureGeneraleQuestion = async () => {
    try {
      const response = await axios.get('https://quoizback2.onrender.com/questions/culture-generale/random');
  ...
  const handleAnswerButtonClick = async (selectedOptionIndex) => {
    ...
    try {
      ...
      const response = await axios.post('https://quoizback2.onrender.com/update-score', { userId: userId, newScore: updatedScore });
